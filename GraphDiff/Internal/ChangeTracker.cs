    ...
    internal class ChangeTracker : IChangeTracker
    {
        ...
        private void EnsureConcurrency(object entity1, object entity2)
        {
            ...
            foreach (var concurrencyProp in concurrencyProperties)
            {
                ...
                {
                    var exceptionType = typeof(DbUpdateConcurrencyException<>);
                    var typeArgument = entity1.GetType();
                    var genericExceptionType = exceptionType.MakeGenericType(typeArgument);
                    var exception = (Exception)Activator.CreateInstance(genericExceptionType, new[] { "A concurrent exception has been occured", entity1, entity2 });
